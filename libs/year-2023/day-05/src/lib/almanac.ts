export function parseAlmanacLines(lines: string[]): [number,number,number][] {
  ...
  lines.forEach(line => {
    map.push(line.split(' ').map(entry => Number.parseInt(entry, 10)) as [number,number,number])
  });
...
export function followMaps(maps: [number,number,number][][], source: number): number {
...
export function reverseFollowMaps(maps: [number,number,number][][], value: number) {
