...
export function runRoundOfMonkeys(
  monkeys: Record<number, Monkey>,
  worryReduceFunction: (num: number) => number
) {
  const totalItems = Object.values(monkeys).reduce(
  ...
  const newTotalItems = Object.values(monkeys).reduce(
    (acc, cur) => (acc += cur.items.length),
    0
  );
  return monkeys;
...
export function calculateMonkeyBusiness(monkeys: Record<number, Monkey>) {
  ...
  const sortedMonkeys = Object.values(monkeys)
