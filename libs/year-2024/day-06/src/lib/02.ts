...
function solveMap(mapData: string[][]): Matrix<string> {
  ...
  while (currentCell !== null) {
    ...
    const nextCell = neighbors.find(
      (neighbor) => neighbor.direction === rotationOrder[currentDirection % 4]
    );
...
function solveMapV2(mapData: string[][]): {
  map: Matrix<string>;
  hasLoop: boolean;
  ...
  const rowObstacles = obstacles.reduce<Record<number, number[]>>(
    (acc, cur) => {
      if (acc[cur.row]) {
  ...
  const colObstacles = obstacles.reduce<Record<number, number[]>>(
    (acc, cur) => {
      if (acc[cur.col]) {
  ...
  while (currentCell !== null) {
    ...
    if (direction === 'n') {
      const nextObstable = colObstacles[currentCell.col]?.filter(
        (colObstacle) => colObstacle < currentCell.row
      );
      ...
      if (nextRow !== undefined) {
        ...
        currentCell = {
          row: nextRow + 1,
          col: currentCell.col,
          value: map.getCellValue(nextRow + 1, currentCell.col),
        };
        currentDirection++;
      } else {
      ...
      const nextRow = colObstacles[currentCell.col]
        ?.filter((colObstacle) => colObstacle > currentCell.row)
        .at(0);
      if (nextRow !== undefined) {
        visitedCells.add(createCellId(currentCell, direction));
        currentCell = {
      ...
      const nextObstacle = rowObstacles[currentCell.row]?.filter(
        (rowObstacle) => rowObstacle > currentCell.col
      );
      ...
      if (nextCol !== undefined) {
        ...
        currentCell = {
          row: currentCell.row,
          col: nextCol - 1,
          value: map.getCellValue(currentCell.row, nextCol - 1),
        };
        currentDirection++;
      } else {
        // console.log(`escaped from ${currentCell}`)
      ...
      const nextCol = rowObstacles[currentCell.row]
        ?.filter((rowObstacle) => rowObstacle < currentCell.col)
        .at(-1);
      if (nextCol !== undefined) {
        visitedCells.add(createCellId(currentCell, direction));
        currentCell = {
    ...
    if (
      currentCell &&
      visitedCells.has(
        createCellId(currentCell, rotationOrder[currentDirection % 4])
      )
    ) {
